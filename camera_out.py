...
class CameraOut:    
    def __init__(self, camera_source=None, model=None, detector=None, customer_data=None, use_device_tokens=None):
    ...
    def run(self):
        ...
        if not self.isrunning:
            self.isrunning = True
            thread.start()
            print("Process camera out run!")
        else:
            print("Process camera out is running already!")
                    ...
                    for idx, face in enumerate(faces):
                        if self.face_is_in_box(boxes[idx]):
                            if self.customer_data.face_ebd_datas.count == 0:
                                ...
                                if face_id != -1:
                                    if not self.customer_data.checkout_list[face_id]:
                                        #TODO
                                        #send noti
                                        print("send noti")
                                        fcm.sendPush("Someone has not paid", f'Someone has not paid, out of door in camera {self.camera_source.cam_name}', self.use_device_tokens.tokens)
                                    del self.customer_data.face_ebd_datas[face_id]
                                    del self.customer_data.checkout_list[face_id]
                        else:
                            frame = self.draw_face(-1, boxes[idx], frame)
            else:
